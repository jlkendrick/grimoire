...
// Decision: the autogenerated tail-print is a convenience and skips nil
// (side-effect spells shouldn't dump "null"); an explicit print: is a
// contract and prints null faithfully.
func TestPrintOp_NilHandling(t *testing.T) {
	calls := 0
	present := func(v any) error { calls++; return nil }

	auto := &PrintOp{Present: present} // no Expr = autogenerated
	if _, err := auto.Run(context.Background(), nil, nil); err != nil {
		t.Fatal(err)
	}
	if calls != 0 {
		t.Error("auto-print must skip a nil value")
	}

	explicit := &PrintOp{Expr: "null", Present: present}
	ex := &Exec{bindings: map[string]any{}}
	if _, err := explicit.Run(context.Background(), nil, ex); err != nil {
		t.Fatal(err)
	}
	if calls != 1 {
		t.Error("explicit print must present null faithfully")
	}
}
